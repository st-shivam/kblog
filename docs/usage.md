...
Multiple `key=value` pairs are ANDed together. Filters apply to any key in the parsed JSON payload.

---

## JSON inspector

When `kblog` detects JSON in a log line, the line is marked. Press `Enter` to open a syntax-highlighted key-value overlay. Press `Enter` or `Esc` to close.
